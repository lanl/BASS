...
y <- rnorm(n, f(x), sigma)
@
We then call function \code{bass} to fit a BASS model using the default settings.
<<c1-2,cache=T,dependson='c1-1'>>=
mod <- bass(x, y)
@
The result is an object that can be used for prediction and sensitivity analysis.  By default, \code{bass} prints progress after each 1000 MCMC iterations, along with the number of basis functions.  To diagnose the fit of the model, we call the \code{plot} function.
